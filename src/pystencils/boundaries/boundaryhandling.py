...
class FlagInterface:
    ...
    def __init__(self, data_handling, flag_field_name, dtype=DEFAULT_FLAG_TYPE):
        ...
        self.flag_field = data_handling.add_array(
            self.flag_field_name, dtype=self.dtype, cpu=True, gpu=False
        )
...
class BoundaryHandling:
    ...
    def __init__(
        ...
        default_dtype: UserTypeSpec = "float64",
        openmp=True,
    ):
        assert data_handling.has_data(field_name)
        assert data_handling.dim == len(
            stencil[0]
        ), "Dimension of stencil and data handling do not match"
        ...
        self.flag_interface = (
            fi if fi is not None else FlagInterface(data_handling, name + "Flags")
        )

        if ParallelDataHandling and isinstance(
            self.data_handling, ParallelDataHandling
        ):
        ...
        data_handling.add_custom_class(
            self._index_array_name, class_, cpu=True, gpu=gpu
        )
    ...
    def set_boundary(
        ...
        for b in self._data_handling.iterate(
            slice_obj, ghost_layers=ghost_layers, inner_ghost_layers=inner_ghost_layers
        ):
            ...
            if mask_callback is not None:
                ...
                if replace:
                    ...
                    np.bitwise_and(
                        flag_arr,
                        ~self.flag_interface.domain_flag,
                        where=mask,
                        out=flag_arr,
                    )
            else:
                if replace:
                    ...
                    np.bitwise_and(
                        flag_arr, ~self.flag_interface.domain_flag, out=flag_arr
                    )
    ...
    def __call__(self, **kwargs):
        ...
        for b in self._data_handling.iterate(gpu=self._target.is_gpu()):
            for b_obj, idx_arr in b[
                self._index_array_name
            ].boundary_object_to_index_list.items():
                kwargs[self._field_name] = b[self._field_name]
                kwargs["indexField"] = idx_arr
                data_used_in_kernel = (
                    p.fields[0].name
                    for p in self._boundary_object_to_boundary_info[
                        b_obj
                    ].kernel.parameters
                    if bool(p.get_properties(FieldBasePtr))
                    and p.fields[0].name not in kwargs
                )
        ...
        for b in self._data_handling.iterate(gpu=self._target.is_gpu()):
            for b_obj, idx_arr in b[
                ...
                data_used_in_kernel = (
                    ...
                    if bool(p.get_properties(FieldBasePtr))
                    and p.fields[0].name not in arguments
                )
                arguments.update(
                    {
                        name: b[name]
                        for name in data_used_in_kernel
    ...
    def geometry_to_vtk(
        self, file_name="geometry", boundaries="all", ghost_layers=False
    ):
        """
        ...
        """
        if boundaries == "all":
            boundaries = list(self._boundary_object_to_boundary_info.keys()) + [
                "domain"
            ]
        ...
        writer = self.data_handling.create_vtk_writer_for_flag_array(
            file_name,
            self.flag_interface.flag_field_name,
            masks_to_name,
    ...
    def _add_boundary(self, boundary_obj, flag=None):
        if boundary_obj not in self._boundary_object_to_boundary_info:
            ...
            ast = self._create_boundary_kernel(
                self._data_handling.fields[self._field_name],
                sym_index_field,
                boundary_obj,
            )
            if flag is None:
                flag = self.flag_interface.reserve_next_flag()
            boundary_info = self.BoundaryInfo(
                boundary_obj, flag=flag, kernel=ast.compile()
            )
            self._boundary_object_to_boundary_info[boundary_obj] = boundary_info
        return self._boundary_object_to_boundary_info[boundary_obj].flag

    def _create_boundary_kernel(
        self, symbolic_field, symbolic_index_field, boundary_obj
    ):
        cfg = CreateKernelConfig()
        cfg.target = self._target
        cfg.default_dtype = self._default_dtype
        cfg.cpu.openmp.enable = self._openmp
        return create_boundary_kernel(
            symbolic_field, symbolic_index_field, self.stencil, boundary_obj, cfg
        )
        ...
        for b in dh.iterate(ghost_layers=ff_ghost_layers):
            ...
            for b_info in self._boundary_object_to_boundary_info.values():
                ...
                idx_arr = create_boundary_index_array(
                    ...
                    boundary_obj,
                    ff_ghost_layers,
                    boundary_obj.inner_or_boundary,
                    boundary_obj.single_link,
                )
                if idx_arr.size == 0:
                    continue

                boundary_data_setter = BoundaryDataSetter(
                    idx_arr, b.offset, self.stencil, ff_ghost_layers, pdf_arr
                )
                index_array_bd.boundary_object_to_index_list[b_info.boundary_object] = (
                    idx_arr
                )
                index_array_bd.boundary_object_to_data_setter[
                    b_info.boundary_object
                ] = boundary_data_setter
                self._boundary_data_initialization(
                    b_info.boundary_object, boundary_data_setter
                )

    def _boundary_data_initialization(
        self, boundary_obj, boundary_data_setter, **kwargs
    ):
...
class BoundaryDataSetter:
    ...
    def __init__(self, index_array, offset, stencil, ghost_layers, pdf_array):
        ...
        assert (
            "x" in arr_field_names
            and "y" in arr_field_names
            and "dir" in arr_field_names
        ), str(arr_field_names)
        self.boundary_data_names = set(self.index_array.dtype.names) - {
            "x",
            "y",
            "z",
            "dir",
    ...
    def non_boundary_cell_positions(self, coord):
        ...
        return (
            self.index_array[self.coord_map[coord]]
            + self.offset[coord]
            - self.ghost_layers
            + 0.5
        )
    ...
    def link_positions(self, coord):
        return (
            self.non_boundary_cell_positions(coord)
            + 0.5 * self.link_offsets()[:, coord]
        )
    ...
    def __setitem__(self, key, value):
        if key not in self.boundary_data_names:
            raise KeyError(
                f"Invalid boundary data name {key}. Allowed are {self.boundary_data_names}"
            )
    ...
    def __getitem__(self, item):
        if item not in self.boundary_data_names:
            raise KeyError(
                f"Invalid boundary data name {item}. Allowed are {self.boundary_data_names}"
            )
...
class BoundaryOffsetInfo:
    ...
    @staticmethod
    def inv_dir(dir_idx):
        return sp.IndexedBase(BoundaryOffsetInfo._untyped_inv_dir_symbol(), shape=(1,))[
            dir_idx
        ]
    ...
    def _offset_symbols(self, dim, dtype: PsIntegerType = SInt(32)):
        return [
            TypedSymbol(f"c{d}", Arr(dtype, len(self._stencil)))
            for d in ["x", "y", "z"][:dim]
        ]
...
def create_boundary_kernel(
    field, index_field, stencil, boundary_functor, cfg: CreateKernelConfig
):
    ...
    config.skip_independence_check = True
    ...
    elements += boundary_functor(
        field, direction_symbol=dir_symbol, index_field=index_field
    )
