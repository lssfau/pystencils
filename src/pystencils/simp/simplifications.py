...
def sort_assignments_topologically(
    assignments: Sequence[Union[Assignment]],
) -> List[Union[Assignment]]:
...
def sympy_cse(ac, **kwargs):
    ...
    all_assignments = [
        e
        for e in chain(ac.subexpressions, ac.main_assignments)
...
def subexpression_substitution_in_existing_subexpressions(ac):
    ...
    for outer_ctr, s in enumerate(ac.subexpressions):
        ...
        for inner_ctr in range(outer_ctr):
            ...
            new_rhs = subs_additive(
                new_rhs, sub_expr.lhs, sub_expr.rhs, required_match_replacement=1.0
            )
...
def add_subexpressions_for_divisions(ac):
    ...
    substitutions = {
        divisor: new_symbol for new_symbol, divisor in zip(new_symbol_gen, divisors)
    }
    return ac.new_with_substitutions(
        substitutions, add_substitutions_as_subexpressions=True, substitute_on_lhs=False
    )
...
def add_subexpressions_for_sums(ac):
    ...
    addends = [
        a
        for a in addends
        if not isinstance(a, sp.Symbol) or isinstance(a, Field.Access)
    ]
    new_symbol_gen = ac.subexpression_symbol_generator
    substitutions = {
        addend: new_symbol for new_symbol, addend in zip(new_symbol_gen, addends)
    }
...
def add_subexpressions_for_field_reads(
    ...
    for fa in field_reads:
        ...
        loads.append(Assignment(substitutions[fa], rhs))
...
def transform_rhs(assignment_list, transformation, *args, **kwargs):
    ...
    return [
        (
            type(a)(a.lhs, transformation(a.rhs, *args, **kwargs))
            if hasattr(a, "lhs") and hasattr(a, "rhs")
            else a
        )
...
def transform_lhs_and_rhs(assignment_list, transformation, *args, **kwargs):
    return [
        (
            type(a)(
                transformation(a.lhs, *args, **kwargs),
